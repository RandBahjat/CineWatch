...
let TOP_10_TRENDING_TODAY = ["Neagley",
  "Resident Evil","The End of Oak Street","Reacher","Spider-Man: Brand New Day","The Fix" , "You+Me – Against the World","Slow Horses", "Lanterns","MobLand"];
...
function renderPlayerDetailsPanel(movie, epData) {
  ...
  const titleEl = document.getElementById("playerPanelTitle");
  const subTitleEl = document.getElementById("playerPanelSubTitle");
  const epBadgeEl = document.getElementById("playerPanelEpBadge");
  const ratingEl = document.getElementById("playerPanelRating");
  const dateEl = document.getElementById("playerPanelDate");
  const durationEl = document.getElementById("playerPanelDuration");
  const episodesSection = document.getElementById("playerPanelEpisodesSection");
  const seasonSelect = document.getElementById("playerPanelSeasonSelect");
  const episodesList = document.getElementById("playerPanelEpisodesList");
  const desktopEpsBtn = document.getElementById("playerEpisodesToggleBtn");

  if (!movie) {
    if (titleEl) titleEl.textContent = state.currentPlayingMovie?.title || "Playing";
  ...
  // Rating & Meta
  if (ratingEl) {
    const r = movie.rating || movie.vote_average;
    ratingEl.textContent = r ? `★ ${Number(r).toFixed(1)}` : "";
  }
  if (dateEl) {
    dateEl.textContent = movie.year || (movie.releaseDate ? movie.releaseDate.split("-")[0] : "");
  }
  if (durationEl) {
  ...
  if (epData && movie.seasons && movie.seasons.length > 0) {
    ...
    if (desktopEpsBtn) desktopEpsBtn.classList.remove("hidden");
    ...
    if (seasonSelect) {
      ...
      seasonSelect.onchange = () => {
        renderPanelEpisodes(parseInt(seasonSelect.value));
      };
    }

    function renderPanelEpisodes(seasonNum) {
      if (!episodesList) return;
      const sData = movie.seasons.find(s => s.season === seasonNum);
      if (!sData) return;
      ...
      episodesList.innerHTML = sData.episodes.map(ep => {
        ...
        return `
          <div class="panel-ep-card ${isCurrent ? "active" : ""}" data-season="${seasonNum}" data-episode="${ep.episode}">
            <div class="panel-ep-thumb-box">
              ${thumb ? `<img src="${thumb}" alt="${ep.title}" loading="lazy" class="panel-ep-thumb-img">` : ""}
              <span class="panel-ep-badge">E${ep.episode}</span>
              <div class="panel-ep-play-icon">▶</div>
            </div>
            <div class="panel-ep-info">
              <h4 class="panel-ep-title notranslate" translate="no">${ep.title}</h4>
              <div class="panel-ep-meta">
                ${isCurrent ? `<span class="panel-ep-playing-tag">● NOW PLAYING</span>` : ""}
                ${rVal ? `<span class="panel-ep-rating">★ ${rVal}</span>` : ""}
      ...
      // Wire episode clicks
      episodesList.querySelectorAll(".panel-ep-card").forEach(card => {
        card.onclick = (e) => {
          e.stopPropagation();
          const sNum = parseInt(card.dataset.season);
          const eNum = parseInt(card.dataset.episode);
          const targetEp = sData.episodes.find(x => x.episode === eNum);
          if (!targetEp) return;

          const tmdbId = movie.videoUrl || movie.cinesrcId || movie.id;
          const absEp = targetEp.absoluteEpisode || '';
          const aniId = movie.anilistId || '';
          const epUrl = targetEp.videoUrl || `tv_embed:${tmdbId}:${sNum}:${eNum}:${absEp}:${aniId}`;
    ...
    // Movie
    if (epBadgeEl) epBadgeEl.classList.add("hidden");
    if (subTitleEl) subTitleEl.textContent = movie.overview ? (movie.overview.length > 120 ? movie.overview.slice(0, 120) + "..." : movie.overview) : "";
    if (episodesSection) episodesSection.classList.add("hidden");
    if (desktopEpsBtn) desktopEpsBtn.classList.add("hidden");
  }

  // Sync Server Pills in Panel
  const curSrv = localStorage.getItem("cw_selected_server") || "mapple";
  syncServerPillsUI(curSrv);
}
...
function updateIframeServer(serverOverride) {
  ...
  if (activeServer === 'mapple') {
    ...
    if (data.type === 'tv') {
      newUrl = `https://mapple.fun/watch/tv/${data.id}-${data.season}-${data.episode}?autoPlay=true&title=true&poster=true&nextButton=true&theme=E74C3C`;
    } else {
